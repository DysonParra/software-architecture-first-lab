 ...
 */
@Stateless
public class SolicitudesManager implements SolicitudesManagerLocal {

    @PersistenceContext(unitName = "com.project.dev_software-architecture-first-lab-ejb_ejb_1.0-SNAPSHOTPU")
    private EntityManager em;

    @Override
    public List<Solicitudes> getAllEntities() {
        Query query = em.createNamedQuery("Solicitudes.findAll");
        return query.getResultList();
    }

    @Override
    public Solicitudes update(Solicitudes solicitudes) {
        return em.merge(solicitudes);
    }
